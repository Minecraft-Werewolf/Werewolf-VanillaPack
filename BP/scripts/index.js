...
// scripts/StandardRoles/constants/scriptevent.ts
...
// scripts/StandardRoles/constants/systems.ts
...
// scripts/StandardRoles/constants/translate.ts
var WEREWOLF_STANDARDROLES_TRANSLATE_IDS = {
  ...
  KNIGHT_SKILL_FORM_TARGET_DROPDOWN_LABEL: "werewolf-standardroles.skill.knight.form.target.dropdown.label",
  KNIGHT_SKILL_PROTECT_RESULT: "werewolf-standardroles.skill.knight.divination.result",
  KNIGHT_SKILL_NO_AVAILABLE_TARGETS_ERROR: "werewolf-standardroles.skill.knight.noAvailableTargets.error"
};

// scripts/StandardRoles/data/roles.ts
var roles = [
  {
    id: "villager",
...
// scripts/StandardRoles/game/events/BaseEventManager.ts
var BaseEventManager = class {
  constructor() {
  }
};

// scripts/StandardRoles/game/ingame/events/EntityHurt.ts
import { world as world3 } from "@minecraft/server";

// scripts/StandardRoles/game/events/BaseEventHandler.ts
...
var InGameEntityHurtHandler = class _InGameEntityHurtHandler extends BaseEventHandler {
  constructor(inGameEventManager) {
    ...
    this.afterEvent = world3.afterEvents.entityHurt;
  }
  ...
  handleAfter(ev) {
    ...
    const hurtEntityData = this.inGameEventManager.getInGameManager().getSelfPlayerData(hurtEntity.id);
    if (!hurtEntityData) return;
    if (hurtEntityData.isProtected) {
      hurtEntityData.isProtected = false;
      for (let i = 0; i < 5; i++) {
        world3.getDimension(MinecraftDimensionTypes.Overworld).spawnParticle("minecraft:totem_particle", {
...
// scripts/StandardRoles/game/ingame/events/InGameEventManager.ts
...
// scripts/StandardRoles/data/skills/knight.ts
import { world as world4 } from "@minecraft/server";
import { ModalFormData } from "@minecraft/server-ui";
var knightSkillHandlers = {
  "knight-protect": async (ev) => {
    const { playerData: pd, playersData: psd, werewolfGameData: we } = ev;
    const player = world4.getPlayers().find((p) => p.id === pd.playerId);
    ...
    if (targetPlayersData.length === 0) {
      ...
      player.sendMessage({
        translate: WEREWOLF_STANDARDROLES_TRANSLATE_IDS.KNIGHT_SKILL_NO_AVAILABLE_TARGETS_ERROR
      });
      return false;
    }
    const form = new ModalFormData().title({
      ...
      {
        translate: WEREWOLF_STANDARDROLES_TRANSLATE_IDS.KNIGHT_SKILL_FORM_TARGET_DROPDOWN_LABEL
      },
...
// scripts/StandardRoles/data/skills/medium.ts
import { world as world5 } from "@minecraft/server";
import { ModalFormData as ModalFormData2 } from "@minecraft/server-ui";

// scripts/StandardRoles/data/skills/utils.ts
function findRoleDefinition(RolesByAddon, RoleId) {
  for (const roles2 of Object.values(RolesByAddon)) {
    const found = roles2.find((r) => r.id === RoleId);
    if (found) return found;
  }
  return void 0;
}
function findFactionDefinition(factionsByAddon, factionId) {
  for (const factions2 of Object.values(factionsByAddon)) {
    const found = factions2.find((f) => f.id === factionId);
    if (found) return found;
  }
  return void 0;
}
function getRoleDefaultColor(c, role) {
  const faction = findFactionDefinition(c.factionDefinitions, role.factionId);
  return faction?.defaultColor ?? "\xA7f";
}

// scripts/StandardRoles/data/skills/medium.ts
var mediumSkillHandlers = {
  "medium-clairvoyance": async (ev) => {
    const { playerData: pd, werewolfGameData: we, ingameConstants: c } = ev;
    const player = world5.getPlayers().find((p) => p.id === pd.playerId);
    if (!player) return false;
    const targetPlayersData = we.playersData.filter(
      (p) => !p.isAlive && p.player.id !== pd.playerId
    );
    ...
    const form = new ModalFormData2().title({
    ...
    player.sendMessage({
      ...
      with: {
        rawtext: [
          ...
          {
            text: clairvoyanceResultRoleDefinition.color ?? getRoleDefaultColor(c, clairvoyanceResultRoleDefinition)
          },
...
// scripts/StandardRoles/data/skills/seer.ts
import { world as world6 } from "@minecraft/server";
import { ModalFormData as ModalFormData3 } from "@minecraft/server-ui";
var seerSkillHandlers = {
  "seer-divination": async (ev) => {
    const { playerData: pd, werewolfGameData: we, ingameConstants: c } = ev;
    const player = world6.getPlayers().find((p) => p.id === pd.playerId);
    if (!player) return false;
    const targetPlayersData = we.playersData.filter(
      (p) => p.isAlive && p.player.id !== pd.playerId
    );
    if (targetPlayersData.length === 0) {
      ...
      player.sendMessage({
        translate: WEREWOLF_STANDARDROLES_TRANSLATE_IDS.SEER_SKILL_NO_AVAILABLE_TARGETS_ERROR
      });
      return false;
    }
    const form = new ModalFormData3().title({
      translate: WEREWOLF_STANDARDROLES_TRANSLATE_IDS.SEER_SKILL_FORM_TITLE
    }).label({ translate: WEREWOLF_STANDARDROLES_TRANSLATE_IDS.SEER_SKILL_FORM_DESCRIPTION }).dropdown(
      {
    ...
    player.sendMessage({
      ...
      with: {
        rawtext: [
          { text: targetPlayerData.player.name },
          {
            text: divinationResultRoleDefinition.color ?? getRoleDefaultColor(c, divinationResultRoleDefinition)
          },
...
// scripts/StandardRoles/data/skills/skillHandlers.ts
var roleSkillHandlers = {
  seer: seerSkillHandlers,
  medium: mediumSkillHandlers,
  knight: knightSkillHandlers
};

// scripts/StandardRoles/game/ingame/game/SkillManager.ts
var SkillManager = class _SkillManager {
  constructor(inGameManager, roles2) {
    this.inGameManager = inGameManager;
  ...
  static create(inGameManager, roles2) {
    return new _SkillManager(inGameManager, roles2);
  }
  async emitPlayerEvent(playerId, eventType) {
    ...
    const playerData2 = werewolfGameData.playersData.find((pd) => pd.player.id === playerId);
    if (!playerData2?.role) {
      return KairoUtils.buildKairoResponse({}, false, "Player has no role");
    }
    const binding = playerData2.role.handleGameEvents?.[eventType];
    if (!binding) {
    ...
    const handlerMap = this.handlersByRoleId.get(playerData2.role.id);
    if (!handlerMap) {
      return KairoUtils.buildKairoResponse(
        { success: false },
        false,
...
// scripts/StandardRoles/data/player.ts
var playerData = {
  playerId: "",
  // playerId は各プレイヤーのIdが自動で割り振られます。(このままにしておいてください)
  isProtected: false
  // 騎士の守護
};

// scripts/StandardRoles/game/ingame/InGameManager.ts
import { world as world8 } from "@minecraft/server";

// scripts/StandardRoles/game/ingame/game/GameManager.ts
...
var GameManager = class _GameManager {
  ...
  runTick() {
    ...
    for (const playerData2 of playersData) {
      this.onTickHandler({
...
// scripts/StandardRoles/data/update.ts
import { world as world7 } from "@minecraft/server";
var onTickUpdate = (ev) => {
  const { playerData: playerData2, playersData, ingameConstants } = ev;
  const players = world7.getPlayers();
  players.forEach((player) => {
    const playerData3 = playersData.find((pd) => pd.playerId === player.id);
    if (!playerData3) return;
    if (playerData3.isProtected) {
...
// scripts/StandardRoles/game/outgame/events/SciprtEventReceive.ts
...
// scripts/StandardRoles/game/outgame/events/OutGameEventManager.ts
...
// scripts/StandardRoles/game/outgame/OutGameManager.ts
...
// scripts/StandardRoles/game/system/events/SystemEventManager.ts
...
// scripts/StandardRoles/data/factions.ts
var factions = [
  {
    id: "villager",
...
// scripts/StandardRoles/game/system/factions/FactionRegistrationRequester.ts
var FactionRegistrationRequester = class _FactionRegistrationRequester {
  constructor(factionManager) {
    this.factionManager = factionManager;
  }
  static create(factionManager) {
    return new _FactionRegistrationRequester(factionManager);
  }
  request() {
...
// scripts/StandardRoles/game/system/factions/FactionManager.ts
var FactionManager2 = class _FactionManager {
  constructor(systemManager) {
    this.systemManager = systemManager;
...
// scripts/StandardRoles/game/system/roles/RoleRegistrationRequester.ts
var RoleRegistrationRequester = class _RoleRegistrationRequester {
  constructor(roleManager) {
    this.roleManager = roleManager;
  }
  static create(roleManager) {
    return new _RoleRegistrationRequester(roleManager);
  }
  request() {
    KairoUtils.sendKairoCommand(
      KAIRO_COMMAND_TARGET_ADDON_IDS2.WEREWOLF_GAMEMANAGER,
...
// scripts/StandardRoles/game/system/roles/RoleManager.ts
var RoleManager2 = class _RoleManager {
  constructor(systemManager) {
    this.systemManager = systemManager;
...
// scripts/StandardRoles/game/system/ScriptEventReceiver.ts
  ...
  handleWorldStateChange(newState, ingameConstants) {
    switch (newState) {
      case SCRIPT_EVENT_MESSAGES2.IN_GAME:
        this.systemManager.changeWorldState(1 /* InGame */, ingameConstants);
        break;
...
// scripts/StandardRoles/game/system/WorldStateChanger.ts
...
// scripts/StandardRoles/game/SystemManager.ts
var _SystemManager = class _SystemManager {
  constructor() {
    ...
    this.factionManager = FactionManager2.create(this);
    this.roleManager = RoleManager2.create(this);
  }
  init() {
